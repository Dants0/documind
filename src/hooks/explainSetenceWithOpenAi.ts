export const explainSetenceWithOpenAi = async (text: string, apiKey: string): Promise<{ explain: string; }> => {
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `Analise a seguinte sentença "${truncatedText}":\\n\nResponda SEMPRE em texto direto, nunca em JSON. Forneça uma explicação breve (máx. 2 linhas) ambo em texto corrido, claro, objetivo e sucinto. Use Markdown para destacar títulos e tópicos, se necessário.`
        }
      ],
      max_tokens: 2000,
      temperature: 0.7,
